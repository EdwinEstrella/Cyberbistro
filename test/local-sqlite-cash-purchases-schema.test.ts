...
describe("tenant-pinned local cash purchase SQLite schema", () => {
  ...
  it("supports an all-or-nothing local purchase graph and sync outbox transaction", () => {
    ...
    try {
      ...
      database.prepare("INSERT INTO gastos (id, tenant_id, sucursal_id, compra_id, payment_method, amount, local_status) VALUES (?, ?, ?, ?, ?, ?, ?)").run("expense-a", "tenant-a", "branch-a", "purchase-a", "cash", 10, "pending_sync");
