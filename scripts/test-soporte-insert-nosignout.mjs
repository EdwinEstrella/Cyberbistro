...
async function run() {
  ...
  // En isServerMode tenemos que setear el token manualmente en supabase (esto es lo que haría useAuth)
  const tokenManager = supabase.tokenManager;
  tokenManager?.setAccessToken?.(adminAuth.accessToken);
  supabase.getHttpClient().setAuthToken(adminAuth.accessToken);
  ...
  console.log("Inserting into tenant_users WITHOUT sign out/sign in...");
