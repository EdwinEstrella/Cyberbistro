...
function parseOrdersCommand(payload: unknown): OrdersCommand | null {
  ...
  if (command.type === "orders.cycle.open" && only("type", "id", "businessDay", "openingCash", "cycleNumber") && text("id") && /^\d{4}-\d{2}-\d{2}$/.test(String(command.businessDay)) && money("openingCash") && Number.isInteger(command.cycleNumber) && (command.cycleNumber as number) > 0) return command as OrdersCommand;
