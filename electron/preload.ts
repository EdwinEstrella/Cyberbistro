...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  activateTenant: (tenantId: string) => ipcRenderer.invoke('tenant-store:activate', tenantId),
  ...
  getInvoiceNumberFloor: (request: unknown) => ipcRenderer.invoke('facturas:number-floor', request),
  deleteInvoiceLocal: (request: unknown) => ipcRenderer.invoke('facturas:delete-local', request),
  saveInvoiceLocal: (invoice: unknown) => ipcRenderer.invoke('facturas:save-local', invoice),
  commitCheckout: (command: unknown) => ipcRenderer.invoke('checkout:commit', command),
  listCierres: (filter?: unknown) => ipcRenderer.invoke('cierres:list', filter),
  listMesasEstado: (filter?: unknown) => ipcRenderer.invoke('mesas:list', filter),
  saveMesaEstado: (payload: unknown) => ipcRenderer.invoke('mesas:save', payload),
  listCocinaEstado: (filter?: unknown) => ipcRenderer.invoke('cocina:status', filter),
  listComandas: (filter?: unknown) => ipcRenderer.invoke('comandas:list', filter),
  saveComanda: (payload: unknown) => ipcRenderer.invoke('comandas:save', payload),
  deleteComanda: (payload: unknown) => ipcRenderer.invoke('comandas:delete', payload),
  listConsumos: (filter?: unknown) => ipcRenderer.invoke('consumos:list', filter),
  saveConsumo: (payload: unknown) => ipcRenderer.invoke('consumos:save', payload),
  deleteConsumo: (payload: unknown) => ipcRenderer.invoke('consumos:delete', payload),
  ...
  listCuentasPagar: (filter?: unknown) => ipcRenderer.invoke('payables:list', filter),
  listCxpPagos: (filter?: unknown) => ipcRenderer.invoke('cxp-pagos:list', filter),
  listExpenseCategories: () => ipcRenderer.invoke('expense-categories:list'),
  syncCloudExpenses: (expenses: unknown[], defaultBranchId?: string) => ipcRenderer.invoke('expenses:sync-cloud', expenses, defaultBranchId),
  syncCloudExpenseCategories: (categories: unknown[]) => ipcRenderer.invoke('expense-categories:sync-cloud', categories),
