...
export function CuentasCobrar() {
  ...
  const cargarDatos = useCallback(async () => {
    ...
    try {
      ...
      if (useLocal) {
        cuentasData = (await readLocalCuentasCobrar(tenantId)) as unknown as CuentaCobrarRow[];
        pagosData = (await readLocalCxcPagos(tenantId)) as unknown as CxcPagoRow[];
        ...
        const [cRes, paRes, cuRes, fRes, cyRes] = await Promise.all([
          ...
          supabase.from("facturas").select("id, numero_factura").eq("tenant_id", tenantId),
